...
if [[ "${SHOULD_START_RABBIT}" == "yes" ]] ; then
    if [[ "${KAFKA}" == "yes" ]] ; then
        ...
        docker run -d -p 2181:2181 -p 9092:9092 --env KAFKA_ADVERTISED_HOST_NAME="${DEFAULT_HEALTH_HOST}" --env KAFKA_ADVERTISED_PORT=9092 --env KAFKA_ZOOKEEPER_CONNECT=localhost:2181 wurstmeister/kafka
    else
        echo -e "\n\nBooting up RabbitMQ"
